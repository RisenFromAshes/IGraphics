...
Planet g_planet[] = {{320, 240, 6, 0, 5, {256, 50, 50}, NULL},
...
void drawPlanet(int x, int y, Planet planet, int fDrawOrbit, int fDrawSatOrbit)
{
    ...
    if (fDrawOrbit) {
        ...
        iEllipse(x * g_scaleFactor,
                 y * g_scaleFactor,
                 planet.majorAxisLen * g_scaleFactor,
                 planet.minorAxisLen * g_scaleFactor);
    }
    ...
    while (pSat) {
        ...
        if (fDrawSatOrbit) {
            iSetColor(255, 255, 255);
            iEllipse(centerX * g_scaleFactor,
                     centerY * g_scaleFactor,
                     pSat->majorAxisLen * g_scaleFactor,
                     pSat->minorAxisLen * g_scaleFactor);
        }

        iSetColor(pSat->rgb.r, pSat->rgb.g, pSat->rgb.b);
        iFilledCircle(
            satelliteX * g_scaleFactor, satelliteY * g_scaleFactor, satelliteR * g_scaleFactor);
...
void iDraw()
{
    ...
    for (i = 0; i < N_PLANETS; i++) {
        centerX =
            g_sunX + sqrt(pow(g_planet[i].majorAxisLen, 2) - pow(g_planet[i].minorAxisLen, 2));
        centerY = g_sunY;
...
void iKeyboard(unsigned char key)
{
    switch (key) {
        case 'p':
        case 'P': iPauseTimer(0); break;
        case 'r':
        case 'R': iResumeTimer(0); break;

        case PLANET_ORBIT_TOGGLER:
        case PLANET_ORBIT_TOGGLER_CAPS: g_fDrawPlanetOrbit = !g_fDrawPlanetOrbit; break;

        case SATELLITE_ORBIT_TOGGLER:
        case SATELLITE_ORBIT_TOGGLER_CAPS: g_fDrawSatelliteOrbit = !g_fDrawSatelliteOrbit; break;
    }
}
...
void iSpecialKeyboard(unsigned char key)
{
    switch (key) {
        ...
        case GLUT_KEY_LEFT: g_sunX += DEL_SCREEN_X; break;
        case GLUT_KEY_RIGHT: g_sunX -= DEL_SCREEN_X; break;
        case GLUT_KEY_DOWN: g_sunY += DEL_SCREEN_Y; break;
        case GLUT_KEY_UP: g_sunY -= DEL_SCREEN_Y; break;
        case GLUT_KEY_PAGE_UP: g_scaleFactor *= 1.2; break;
        case GLUT_KEY_PAGE_DOWN: g_scaleFactor /= 1.2; break;
